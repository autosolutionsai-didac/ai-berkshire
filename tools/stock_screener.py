...
Framework:
  Layer 1 (momentum discovery): 60-day high + volume confirmation → enters the candidate pool
...
Improvements (from NVDA/AMD/MU backtests):
  1. Gross margin improving for 2 consecutive quarters → standalone buy condition (fixes the NVDA 2023-01 miss)
  2. EPS beat > 30% → standalone condition for cyclical stocks (fixes the MU bottom signal)
...
def fetch_prices_curl(ticker, days=120):
    """Fetch Yahoo Finance daily bars via curl"""
...
def check_momentum(prices):
    ...
    # Volume surge: 5-day avg volume > 20-day avg volume x 1.5
...
# ============================================================
# Layer 2: value validation (6 dimensions, incl. backtest improvements)
# ============================================================
...
def check_value(ticker, signal_date=None):
    ...
    # 1. Revenue acceleration (YoY growth is improving)
    if pd:
        checks["Rev accel"] = d["rev_yoy"] > pd["rev_yoy"]
    else:
        checks["Rev accel"] = d["rev_yoy"] > 20
    ...
    # 3. EPS beat > 10%
    checks["Earnings beat"] = d["eps_beat"] > 10
    ...
    # 6. ★ Improvement: gross margin improving 2 consecutive quarters (fixes the NVDA 2023-01 miss)
    if pd and pd2:
        checks["GM consec improve"] = d["gm"] > pd["gm"] > pd2["gm"]
    elif pd:
        checks["GM consec improve"] = d["gm"] > pd["gm"]
    else:
        checks["GM consec improve"] = False
    ...
    # Condition A: gross margin improving 2 consecutive quarters + GM > 45% (NVDA 2023-01 scenario)
    if checks.get("GM consec improve") and d["gm"] > 45:
        independent_pass = True
        independent_reason = "GM consecutively improving + >45%"
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"  Momentum discovery + value validation stock screener  {today}")
    ...
    if watch_signals:
        ...
        for s in watch_signals:
            m = s["momentum"]
            print(f"     {s['ticker']:<8} ${m['close']:<8} 30d+{m['pct_30d']}% — please run --update {s['ticker']} to add data")
