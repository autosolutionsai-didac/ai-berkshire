...
# (API-fetched quarterly financials are unreliable; manually entering core data is more accurate)
# ============================================================
...
FUNDAMENTALS = {
    "NVDA": {
        ...
        "quarters": OrderedDict([
            # (earnings release date, {revenue in USD 100M, revenue YoY growth, gross margin, EPS, EPS beat %})
...
# ============================================================
# Part 3: Momentum discovery engine (first-layer filter)
# ============================================================
...
def compute_momentum_signals(prices):
    ...
    for i in range(60, len(prices)):
        ...
        # Volume surge confirmation: recent 5-day avg volume > 2x the 20-day avg volume
...
# ============================================================
# Part 4: Value verification engine (second-layer filter)
# ============================================================

def find_latest_fundamental(ticker, signal_date):
    """Find the most recent quarterly earnings before the signal date"""
...
def verify_value(ticker, fund_data, prev_fund_data=None):
    ...
    if not fund_data:
        return {"score": 0, "details": "No fundamentals data"}

    checks = {}

    # 1. Revenue accelerating (whether YoY revenue growth is improving)
    ...
    checks["Revenue accelerating"] = rev_accelerating
    ...
    if prev_fund_data:
        ...
        gm_expanding = gm > 45
    checks["Gross margin expanding"] = gm_expanding
    ...
    # 5. Gross margin absolute value (>40%, chip industry standard)
...
def backtest_ticker(ticker):
    """Run a full backtest on a single ticker"""
    print(f"\n{'='*70}")
    ...
    for sig in momentum_signals:
        ...
        if month_key in seen_months:
            continue  # only take the first signal in a given month
        seen_months.add(month_key)

        # Find fundamentals data
    ...
    # Only show signals within the key time window
    first_buy = None
    for bs in buy_signals:
    ...
    if first_buy and prices:
        ...
        # Find prices 1 year and 2 years later
        for p in prices:
            if p["date"] >= buy_date:
                final_price = p["close"]
        ...
        print(f"\n  {'='*60}")
        print(f"  📊 Assuming execution at the first buy signal:")
...
if __name__ == "__main__":
    ...
    print("  Targets: NVDA / AMD / MU | Time: 2022-2025")
    print("=" * 70)
