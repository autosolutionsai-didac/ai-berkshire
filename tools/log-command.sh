...
# Invoked by the user_prompt_submit hook; reads user input from stdin
...
# Print a reminder every 10 entries (hook stdout is shown to Claude)
if [ $((COUNT % 10)) -eq 0 ]; then
    TOTAL=$(wc -l < "$LOG_FILE" | tr -d ' ')
    echo "[Command log] ${TOTAL} commands recorded so far. Consider running /command-log to add background summaries for recent commands."
fi
