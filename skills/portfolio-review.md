...
- A holdings list, e.g.: `Tencent 30%, Meituan 20%, Kweichow Moutai 20%, NVIDIA 15%, Cash 15%`
...
| Ticker | Code | Position | Cost Basis | Current Price | Market Value | Weight | P&L |
|------|------|--------|-------|------|------|------|------|

If the input provides only percentages and no amounts, analyzing by percentage is sufficient.

At the same time, check whether an existing portfolio file (`reports/portfolio-latest.md`) exists; if so, read it and update it.

### Step 2: Fetch the Latest Data

Use the Task tool to launch background Agents, fetching the following in parallel for each holding via WebSearch:
1. Current share price and valuation metrics (PE, PB, dividend yield)
2. Key financial changes in the most recent quarter
3. Recent material events
4. Analyst consensus estimates (forward PE, price target)

For each holding, use `tools/financial_rigor.py verify-valuation` to validate the valuation data. For each holding, tag the information-richness rating (Grade A/B/C); for Grade-C holdings, flag the analytical conclusions as low-confidence.

### Step 3: Single-Position Health Check

Run a quick health check on each holding:

| Ticker | Current PE | Has the buy thesis changed | Thesis Health | Position Recommendation |
|------|:------:|:--------------:|:---------:|---------|
| Tencent | 18x | Unchanged | 8/10 | Reasonable |
| Meituan | 25x | Competition intensifying | 6/10 | On the high side, consider trimming |

For each holding, answer:
- [ ] **If you held none of it today, would you still buy at the current price?**
- [ ] **If you couldn't trade tomorrow, would you be comfortable holding for 5 years?**
- [ ] **Is the buy thesis still intact?**

**Duan Yongping**: "If you're not willing to hold a stock for ten years, don't hold it for even a day."
...
**Li Lu's standard**: 3-5 core holdings, with the top 3 making up 80%+. **But this requires that every one of them be thoroughly researched.**
...
| NVIDIA | TSMC | AI supply-chain up/downstream | AI capex moves in the same direction |
| Meituan | Pinduoduo (PDD) | Both Chinese consumption | Macro consumption moves in the same direction |
...
- [ ] If the global economy falls into recession, how much would the portfolio lose?

#### 4.3 Opportunity-Cost Analysis

This is Buffett's most fundamental way of thinking—**every dollar should be put where the return is highest**.
...
Method for estimating expected return (compute using `tools/financial_rigor.py three-scenario`):
- **Simplified formula**: expected annualized ≈ FCF Yield + expected growth rate (primary method)
- **Value-style cross-check**: margin-of-safety reversion + earnings growth + dividend yield
- **Growth-style cross-check**: earnings growth × change in a reasonable PE

**Key question**: For the lowest-ranked holding, is the expected return higher than cash (risk-free rate ~4%)? If not, it should be sold and converted to cash.
...
| US-China conflict escalates | China ADRs discounted 50% | | |
| Rates spike | 10-year Treasury → 6% | | |
| Tech bubble bursts | Tech-stock PE compresses 40% | | |

For each scenario, do a qualitative + rough-estimate assessment (based on each holding's industry profile and historical valuation-swing range):
- Which holdings would be hit hardest? Approximate direction and magnitude range of impact
...
| Action | Ticker | Current Weight | Recommended Weight | Rationale |
|------|------|:-------:|:-------:|------|
| Add | | | | |
| Trim | | | | |
| Exit | | | | |
...
If the portfolio contains positions that are "worse than cash," or the cash weight is too high, recommend using `/industry-research` or `/investment-checklist` to systematically screen the industries/companies of interest, rather than directly recommending individual stocks within this Skill.
...
**Buffett**: currently holds $382B in cash, more than 25% of total assets—when no good opportunities can be found, cash is the best position.
...
5. Next review date and key items to watch
```

#### The Conclusion Must Clearly Answer

1. **Overall portfolio health**: Excellent / Good / Needs adjustment / Serious problems
2. **What is the single most important thing to do?** (add to X / trim Y / do nothing)
...
- **Concentration is not the risk, ignorance is** — holding 3 stocks you understand deeply is safer than holding 30 you only half-understand
- **Cash is a position** — there is no shame in holding cash when no good opportunities can be found
- **Portfolio level > single-stock level** — even a good stock will drag you down at the wrong position size
- **Review regularly, but don't overtrade** — reviewing once a quarter is enough; don't watch the ticker and rebalance every day
